...
chúng ta sẽ thấy nó cũng sẽ mang đến sự thuận tiện tương tự (hoặc có thể hơn) cho việc triển khai các mô hình phân loại.
...
Như đã đề cập trong :numref:`sec_softmax`, tầng output của softmax regression là một tầng fully-connected (`Dense`).
Do đó, để cài đặt mô hình, chúng ta chỉ cần thêm một tầng `Dense` với 10 output vào đối tượng `Sequential`.
Ở đây, việc sử dụng `Sequential` là không thực sự cần thiết, nhưng chúng ta nên hình thành thói quen vì nó sẽ có mặt ở khắp nơi khi triển khai các mô hình học sâu.
